...
static void linphone_core_init(LinphoneCore * lc, const LinphoneCoreVTable *vtable, LpConfig *config, void * userdata){
	...
	lc->ringplayer = linphone_ringplayer_new(lc);
...
void linphone_core_notify_incoming_call(LinphoneCore *lc, LinphoneCall *call){
	...
	if (ms_list_size(lc->calls)==1){
		...
		linphone_ringplayer_start(lc->ringplayer, ringcard, lc->sound_conf.local_ring, 2000);
	}else{
...
int linphone_core_accept_call_with_params(LinphoneCore *lc, LinphoneCall *call, const LinphoneCallParams *params){
	...
	/*stop ringing */
	if (linphone_ringplayer_is_started(lc->ringplayer)) {
...
int linphone_core_preview_ring(LinphoneCore *lc, const char *ring,LinphoneCoreCbFunc func,void * userdata)
{
	...
	lc->preview_finished=0;
	return linphone_ringplayer_start_with_cb(lc->ringplayer, ringcard, ring, 2000,notify_end_of_ring,(void *)lc);
}
...
static void linphone_core_uninit(LinphoneCore *lc)
{
	...
	if (lc->ringplayer) {
		linphone_ringplayer_destroy(lc->ringplayer);
	}
...
void linphone_core_stop_ringing(LinphoneCore* lc) {
	LinphoneCall *call=linphone_core_get_current_call(lc);
	if (linphone_ringplayer_is_started(lc->ringplayer)) {
		linphone_ringplayer_stop(lc->ringplayer);
	}
	if (lc->ringstream) {
		ring_stop(lc->ringstream);
		lc->ringstream=NULL;
